      ...
      <table class="data-table">
        <thead>
          <tr class="table-header">
            ...
            <th class="header-cell sortable" @click="toggleSort('sales')">
              销售金额 (元)
              <span class="sort-indicator">{{ getSortIndicator('sales') }}</span>
            </th>
            <th class="header-cell sortable" @click="toggleSort('returns')">
              退货金额 (元)
              <span class="sort-indicator">{{ getSortIndicator('returns') }}</span>
            </th>
            <th class="header-cell sortable" @click="toggleSort('net')">
              净销售额 (元)
              <span class="sort-indicator">{{ getSortIndicator('net') }}</span>
            </th>
            <th class="header-cell">退货率</th>
          </tr>
        </thead>
        <tbody>
          <tr 
          ...
          >
            ...
            <td class="table-cell amount-cell">{{ customer.sales }}</td>
            <td class="table-cell return-cell">{{ customer.returns }}</td>
            <td class="table-cell net-cell" @click="viewCustomerDetail(customer.id)">
              {{ customer.net }}
            </td>
            <td class="table-cell rate-cell" :class="getReturnRateClass(customer.returnRate)">
              {{ customer.returnRate }}
            </td>
          </tr>
        </tbody>
...
export default {
  ...
  computed: {
    sortedCustomers() {
      if (!this.sortField) return this.customers
      ...
      return [...this.customers].sort((a, b) => {
        ...
        // 如果是金额字段，需要转换为数字比较
        if (['sales', 'returns', 'net'].includes(this.sortField)) {
  ...
  methods: {
    ...
    getReturnRateClass(rate) {
      ...
      if (rateNum > 10) return 'high-return'
      if (rateNum > 5) return 'medium-return'
      return 'low-return'
    }
  }
}
...
/* 筛选面板样式（与EmployeePerformance.vue保持一致）*/
.filter-panel {
  background-color: #ffffff;
  border-bottom: 1px solid #eeeeee;
  padding: 16px;
...
.rate-cell.high-return {
  color: #ff4757;
  font-weight: bold;
}

.rate-cell.medium-return {
  color: #ffa726;
  font-weight: 500;
}

.rate-cell.low-return {
  color: #2ed573;
  font-weight: 500;
}
